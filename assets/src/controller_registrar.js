...
export function registerControllers(application, eagerControllers, lazyControllers) {
    ...
    const load = (identifier) => {
        ...
        Promise.resolve()
            .then(loader)
            .then((controller) => register(identifier, controller))
            .catch((error) => console.error(`Error loading controller "${identifier}":`, error))
            .finally(stopWatchingIfComplete);
    };
